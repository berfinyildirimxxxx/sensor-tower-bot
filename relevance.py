...
# ─── MECHANIC GROUPS ────────────────────────────────────────────────────────
# Each group: if ANY keyword matches → +40 points, mechanic is detected
# Two or more matches in same group → capped at +40 (no double counting per group)
...
MECHANIC_GROUPS: dict[str, list[str]] = {
    ...
    "Match-3 / Blast": [
        ...
        "bubble blast",
        "bubble pop",
        "bubble shooter",  # borderline ama match kategorisinde sayılır
        "match gems",
        "match jewels",
        "match candy",
        "match fruit",
...
# ─── SUPPORTING SIGNALS ─────────────────────────────────────────────────────
# These alone don't make a game relevant, but they boost the score
# Each match: +8 points, max +24 total from this section
...
# These strongly indicate the game is NOT in Agave's space
# Each match: -35 points
...
# ─── NAME BONUSES ───────────────────────────────────────────────────────────
# If game NAME contains these → +15
...
def _build_search_text(game: dict[str, Any]) -> str:
    """Combine all relevant fields into one lowercase searchable string."""
...
def _find_mechanic(text: str) -> tuple[str, int]:
    """Find the best matching mechanic and its score contribution.

    Returns (mechanic_name, score_to_add).
    Score per mechanic group: +40 if any keyword matches (not stacked).
    If multiple groups match, take the one with the most keyword hits.
    """
    best_mechanic = "Unknown"
    best_hit_count = 0
    total_mechanic_score = 0
    matched_mechanics: list[tuple[str, int]] = []

    for mechanic, keywords in MECHANIC_GROUPS.items():
        hits = sum(1 for kw in keywords if kw in text)
        if hits > 0:
            matched_mechanics.append((mechanic, hits))
            total_mechanic_score += 40  # each mechanic group adds 40
            if hits > best_hit_count:
                best_hit_count = hits
                best_mechanic = mechanic

    # Cap mechanic score at 80 (max 2 groups contributing)
    capped_score = min(total_mechanic_score, 80)
    return best_mechanic, capped_score


def _supporting_score(text: str) -> int:
    """Score from general supporting keywords. Capped at 24."""
    hits = sum(1 for kw in SUPPORTING_KEYWORDS if kw in text)
    return min(hits * 8, 24)


def _negative_score(text: str) -> int:
    """Penalty from negative keywords. Returns a negative number."""
    hits = sum(1 for kw in NEGATIVE_KEYWORDS if kw in text)
    return hits * -35


def _category_bonus(game: dict[str, Any]) -> tuple[int, list[str]]:
    """Bonus points from game category field."""
...
def _name_bonus(game: dict[str, Any]) -> int:
    """Bonus if game name contains strong puzzle signals."""
...
def score_game(game: dict[str, Any]) -> tuple[int, str, str]:
    ...
    # If heavy negative penalty wiped everything out, mark mechanic as Not Relevant
    ...
    return final_score, reason, mechanic
